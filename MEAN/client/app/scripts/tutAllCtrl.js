...
.controller('tutAllCtrl',['$scope', 'apiService', '$state', 'system', function($scope, apiService, $state, system){
    
    $scope.tableRowClicked = function(item){
        console.log("table row is clicked");  
        console.log(item);
        //$state.go('app.status',{'_id':$scope._id, 'sid':$scope.sid});	
        $state.go('app.tutdetails',{'id':item._id});
    };
            ...
            function(response){
                if (response.status.code == 10000){
                    ...
                    for (var i=0; i<response.data.length; i++){
                        if((response.data[i].tutor_id === $scope.user_id) && (response.data[i].is_graded==false)){
    ...
    $scope.$on('$viewContentLoaded', function(){
        console.log("tutor all view is loaded");
    ...
    $scope.$on('$destroy', function(){
        console.log("tutor all is destroy"); 
    });
